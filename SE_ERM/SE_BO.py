...
def Integrand_out(y, z, w, V)
    return 

def Z_out(y, z, w, V, IntBoundaries):
    return 

def z_avg_out(y, z, w, V, IntBoundaries):
    return  1 / Z_out(y, z, w, V, IntBoundaries)

def T(qhat, W, xi):
    sqrtqhatInv = sqrtm(linalg.inv(qhat)).real
    return W + sqrtqhat @ xi
...
def HatFuncs(sigma, z, w, A, SigmaInv, IntBoundaries):
    gOut = gout(z_avg_out(phi(z, A), z, w, V, IntBoundaries), w, sigma)
...
def TrueRandExpectHat(q, alpha, Nsample):
    comm = MPI.COMM_WORLD
    size = comm.Get_size()
    local_N = Nsample // size
    sigma = np.eye(2) - q
    ...
    for _ in range(local_N):
        ...
        newqhat = HatFuncs(sigma, z, w, A, linalg.inv(Sigma))
        qhat += newqhat
...
def TrueRandSE_BO(alpha, q0, Damping, Nsample, MaxIter, EpsConvergence, Verbose, VerboseRate, DebugVerbose):
    comm = MPI.COMM_WORLD
    rank = comm.Get_rank()

    q = q0
    qhat= TrueRandExpectHat(q, alpha, Nsample)
    NIter = 0
    Conv = 1
    while((Conv > EpsConvergence) and (NIter < MaxIter)):
        IterStart = time.time()
        newqhat = TrueRandExpectHat(q, alpha, Nsample)
...
def main():
    ...
    StartTime = time.time()
    q, varq = TrueRandSE_BO(args.alpha, q0, Damping = args.Damping, Nsample = args.Nsample, MaxIter = args.MaxIter,
